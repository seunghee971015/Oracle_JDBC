...
SELECT EMP_NAME, SALARY * 12, HIRE_DATE, PHONE
FROM EMPLOYEE
WHERE SAL_LEVEL = S1;
...
SELECT EMP_NAME AS "이름" , SUBSTR(EMP_NO,1, 6) AS "생년, 생월, 생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, FLOOR(SYSDATE - HIRE_DATE)  AS "근무일수1" , ABS(TRUNC(HIRE_DATE - SYSDATE)) AS "근무일수2" 
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, '9,999,999')||'원'
FROM EMPLOYEE;
--21. EMPLOYEE테이블에서 직원 명, 부서코드, 생년월일, 나이(만) 조회
-- (단, 생년월일은 주민번호에서 추출해서 00년 00월 00일로 출력되게 하며
-- 나이는 주민번호에서 출력해서 날짜데이터로 변환한 다음 계산)
SELECT EMP_NAME, --EXTRACT(YEAR FROM TO_DATE(SUBSTR (EMP_NO, 1, 6)))||'년 ' || 
                    --EXTRACT(MONTH FROM TO_DATE(SUBSTR (EMP_NO, 1, 6)))||'월 ' ||
                    --EXTRACT(DAY FROM TO_DATE(SUBSTR (EMP_NO, 1, 6)))||'일 '
                    --TO_DATE(SUBSTR(EMP_NO, 1, 6)) 
                    TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6))  ,'YYYY"년" MM"월" DD"일"')
